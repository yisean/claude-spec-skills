...
`NNN` 是**扁平三位号**，层级到此为止——**没有子号**（不存在 `001.1`）。一个 `NNN` = 一份 PRD = 「一句话能说清要解决谁的什么问题」的一个特性。所以一个大 feature（epic）讨论需求时冒出子功能，不给子号，而是按「子功能能不能独立交付」二选一：

| | A · 留在一份 PRD | B · 各自拆成顶层 NNN（Scrum 多人并行优先） |
| --- | --- | --- |
| 何时 | 子功能耦合、一起上线、一份设计/计划装得下 | 子功能**能独立交付/单独上线** |
| 子功能表达为 | 一份 PRD 里的 `R/F`（R1、R2…） | 各取顶层 `NNN`（001、002…），各自 PRD/设计/计划 |
| 多人并行粒度 | 同一 PRD 内按 `U`（实现单元）切分认领 | **NNN 级并行**——每人 registry 各取一个号、各开分支 |
| 子号防撞 | `R/F`·`AE/AC`·`U` rebase 续编 + C1 兜底 | 各自独立号段，天然不撞 |

> **判据**：子功能能不能单独上线？能 → B（扁平多号）；不能、只是同一特性的一条需求 → A（一份 PRD 当 `R/F`）。
> **已给 epic 取了号又决定走 B**：那个号别让 epic 占着——改 slug 指向**第一个具体子功能**（还没产出物引用它，可转用），其余子功能续取 002、003；epic 退回主题，不占号。

**epic 用 slug 前缀分组，不用号表达层级**。registry 的 `slug` 列给同一 epic 的子功能加**共同前缀**（`<epic>-<子功能>`，kebab-case），一眼归堆、`/spec-check` C16 跨特性检索也好认：

```
| NNN | slug                   | owner | date       | status   | 说明 |
| 001 | booking-room-reserve   | 张三  | 2026-06-09 | reserved | 会议室预约-员工预约/取消 |
| 002 | booking-room-admin     | 李四  | 2026-06-09 | reserved | 会议室预约-管理员维护会议室 |
| 003 | booking-room-notify    | 王五  | 2026-06-10 | reserved | 会议室预约-预约通知 |
```

> 这里 epic 是「会议室预约（booking-room）」这个**主题**，不是一个号；三个子功能各占顶层 `NNN`，三人各开 `feat/001-booking-room-reserve` 等分支并行。epic 维度若要正式记录，放 `STRATEGY.md` 的 track，不进 registry。
>
> ⚠️ 别自创 `001.1`：文件命名 `YYYY-MM-DD-NNN-slug`、registry、spec-check C8b/C16、覆盖矩阵都按扁平号匹配，子号会让全链路对不上。**要层级就「扁平 NNN + slug 前缀」。**
