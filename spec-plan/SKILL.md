...
- **非功能约束承接**：把 PRD 的非功能约束（性能/并发/安全权限/兼容性）落到具体设计手段，每条对应一个实现或校验点，不让 NFR 停在 PRD 里。
- **风险与回滚**：高风险点、并发/权限/性能注意项。

### Phase 2 · 数据 ER 模型

把数据设计画成 **Mermaid `erDiagram`**（逻辑视图）：实体、关系、关键字段、主外键、唯一约束都要体现。**可填骨架与 `erDiagram` 示例见本 skill 目录 `templates/plan.md` 的「数据 ER 模型」节**。ER 是**逻辑视图**，与 Phase 3 的 migration sql（**物理脚本**）一一对应、逐字段一致。
...
- 正文：Summary → Problem Frame → **概要设计**（Phase 1）→ **数据 ER 模型**（Phase 2 的 mermaid）→ **DB 迁移**（引用 `docs/ops/install/migration-YYYY-<特性名>.sql`）→ Requirements 映射 + **覆盖矩阵**（Phase 4 三向表）→ **Implementation Units**（含详细设计的 Phase 4 单元）。
