...
依**设计文档的 ER 模型**，把数据库改动落到 `docs/ops/install/migration-YYYY-<特性名>.sql`：建表/加字段/索引/唯一键/种子数据，写清安全性与可回滚方式，并在相关任务里引用该脚本。**确保与设计的 ER 模型逐字段一致**（ER 是逻辑视图，migration 是其物理实现，本计划不重画 ER 只落地它）。
