...
- 正文：Summary → Problem Frame → **概要设计**（Phase 1）→ **数据 ER 模型**（Phase 2 的 mermaid）→ **DB 迁移**（引用 migration sql）→ Requirements 映射 + **覆盖矩阵**（Phase 4 三向表）→ **Implementation Units**（含详细设计的 Phase 4 单元）。
